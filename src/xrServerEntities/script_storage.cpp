...
class ScriptSourceCache final : xray::noncopyable
{
	...
	bool Compile(const ScriptPrefetchItem& item, const std::shared_ptr<ScriptSourceEntry>& entry, lua_State* compiler)
	{
	...
	void Worker()
	{
		lua_State* compiler = luaL_newstate();
		for (;;)
		{
			...
			if (EnsureLoaded(item.path.c_str(), entry, false))
				Compile(item, entry, compiler);
		}
		if (compiler)
			lua_close(compiler);
		...
		if (active_workers.fetch_sub(1, std::memory_order_acq_rel) == 1)
		{
			...
			Msg("* [lua-cache] source=%u/%u failed=%u bytecode=%u failed=%u source-bytes=%llu KB bytecode-bytes=%llu KB elapsed=%llu ms",
...
public:
	...
	void StartPrefetch()
	{
		...
		Msg("* [lua-cache] start files=%u workers=%u mode=source+bytecode",
	...
	void LogStats() const
	{
		Msg("* [lua-cache] status source=%u/%u failed=%u bytecode=%u failed=%u active=%u demand-misses=%u demand-waits=%u bytecode-hits=%u bytecode-misses=%u source-bytes=%llu KB bytecode-bytes=%llu KB",
			ready_files.load(std::memory_order_relaxed),
			total_files.load(std::memory_order_relaxed),
			failed_files.load(std::memory_order_relaxed),
			bytecode_ready.load(std::memory_order_relaxed),
