...
void CRenderTarget::phase_accumulator()
{
	...
	if (dwAccumulatorClearMark == Device.dwFrame)
	{
		...
		{
			if (RImplementation.o.fp16_blend) u_setrt(rt_Accumulator, NULL,NULL, main_depth());
			else u_setrt(rt_Accumulator_temp, NULL,NULL, main_depth());
		}
		else
		{
...
void CRenderTarget::phase_vol_accumulator()
{
	...
	else
	{
		...
		if (!m_bHasActiveVolumetric)
		{
			m_bHasActiveVolumetric = true;
			if (!RImplementation.o.dx10_msaa)
				u_setrt(rt_Generic_2, NULL, NULL, main_depth());
			else
