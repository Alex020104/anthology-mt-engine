...
ENGINE_API void EngineRecordSuppressedLuaJITFlush()
{
	...
	else if (!g_postload_jit_flush_reported)
	{
		g_postload_jit_flush_reported = true;
		Msg("* [lua-maintenance] retained post-load LuaJIT traces");
	}
}
//-AVO
...
bool CApplication::LoadSessionCanReuseStartupLua() const
{
	return m_load_session.active && !xr_strcmp(m_load_session.scenario, "menu-save") &&
		!Core.ParamsData.test(ECoreParams::reload_menu_lua);
}
...
void CApplication::LoadSessionPrecacheBegin()
{
	...
	m_load_session.precache_scheduler_ticks = 0;
}
...
void CApplication::LoadSessionRecordPrecacheScheduler(u64 ticks)
{
	if (!m_load_session.active || !m_load_session.precache_started)
		return;

	m_load_session.precache_scheduler_ticks += ticks;
}
...
void CApplication::LoadSessionTryFinish(bool level_ready, bool control_ready, bool queues_drained)
{
	...
	Msg("* [load-session] precache perf: frames=%u, calls(level/loadscreen/present)=%u/%u/%u, "
		...
		to_ms(m_load_session.precache_scheduler_ticks),
