...
void CObjectList::Update(bool bForce)
{
		...
		if (Device.fTimeDelta > EPS_S || bForce)
		{
...

			{
...

				{
					...
					if (workload.capacity() < required_workload->size())
						workload.reserve(required_workload->size());
					workload.assign(required_workload->begin(), required_workload->end());
				}
	...
	if (measure_detail)
	{
		...
		if (object_update_profile.frames >= 300)
		{
			...
			Msg("* [mt-frame/profile] objects avg(list/updatecl)=%.2f/%.2f ms "
				"calls/frame=%.2f workload/frame=%.2f slowest=%s/%s %.2f ms",
