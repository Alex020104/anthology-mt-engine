...
void CSheduler::ProcessStep()
{
	...
	int i = 0;
	{
		for (i = 0; i < ItemsBatch.size(); ++i)
		{
			...
			// Keep the existing item and its diagnostic name. Rebuilding it here
			// called shedule_Name() for hundreds of objects on every populated-base
			// frame even when scheduler logging was disabled.
	...
	// Reinsertion
	{
		...
		// complete rebuild costs O(n). Use the actual heap depth instead of the
		// old n/16 approximation which rebuilt Anthology's ~3000 item heap for a
		// normal 256-object base batch.
