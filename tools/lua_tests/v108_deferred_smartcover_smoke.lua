local patch_path = assert(arg[1], "path to v108 script is required")
...
UnregisterScriptCallback = function(name, callback)
	if callbacks[name] == callback then
		callbacks[name] = nil
	end
end
...
local paths = {
	...
	mob_path = { position = vec(901, 0, 0), vertex = 901 },
}
...
smart_terrain.se_smart_terrain.setup_logic = function(_, npc)
	...
	elseif id == 7 then
		db.storage[id] = {
			active_scheme = "animpoint",
			active_section = "deferred_cover_work",
			animpoint = {
				cover_name = "deferred_cover",
				position = vec(0, 0, 0),
				level_vertex_id = 777,
			},
		}
	...
	elseif id == 9 then
		db.storage[id] = {
			active_scheme = "mob_walker",
			active_section = "mob_work",
			mob_walker = { path_walk = "mob_path" },
		}
	elseif id == 10 then
		db.storage[id] = {
			active_scheme = "animpoint",
			active_section = "observed_cover_work",
			animpoint = { cover_name = "observed_cover" },
		}
	end
end
...
local base = make_smart("base", { 1, 2, 3, 4, 5, 7, 8, 9, 10 })
...
alife = function()
	return {
		object = function(_, id)
			return smarts[id] or server_objects[id]
		end,
	}
end
...
xr_motivator.motivator_binder.net_spawn = function(self, se_obj)
	...
	if redirect then
		-- Model the Exo binder overwriting exact coordinates with the coarse
		-- navigation vertex after smart setup.
...
assert(type(callbacks.server_entity_on_register) == "function", "smart-cover register callback missing")
assert(type(callbacks.actor_on_first_update) == "function", "first-update callback missing")

-- A cover registered before its NPC must be usable even if the stock registry
-- has not published it yet.
callbacks.server_entity_on_register({
	dynamic_name = "observed_cover",
	position = vec(801, 0, 0),
}, "se_smart_cover")

local function spawn(id, smart_id, server_x)
	local object = npc(id)
	local se_obj = { id = id, m_smart_terrain_id = smart_id, position = vec(server_x, 0, 0) }
	online_objects[id] = object
	server_objects[id] = se_obj
...
local deferred_cover = spawn(7, 51, 0)
local zero_cover = spawn(8, 51, 0)
local mob_walker = spawn(9, 51, 0)
local observed_cover = spawn(10, 51, 0)
...
assert(animpoint:position().x == 501, "registered animpoint NPC did not start at its smart-cover")
assert(far_arrival:position().x == -1, "real far arrival was teleported")
assert(deferred_cover:position().x == -1, "unregistered cover moved before it existed")
assert(zero_cover:position().x == -1, "zero smart-cover placeholder moved an NPC")
assert(mob_walker:position().x == -1, "generic mob_walker fallback moved a non-human scheme")
assert(observed_cover:position().x == 801, "observed server cover was not used during spawn")
assert(db.offline_objects[1] == nil, "patch mutated persistent offline-object state")

-- The real load order can register a smart-cover after its worker.  Capture it
-- and resolve once on actor_on_first_update while the load screen is still up.
callbacks.server_entity_on_register({
	dynamic_name = "deferred_cover",
	position = vec(701, 0, 0),
}, "se_smart_cover")
callbacks.server_entity_on_register({
	dynamic_name = "zero_cover",
	position = vec(0, 0, 0),
}, "se_smart_cover")
callbacks.actor_on_first_update()

assert(deferred_cover:position().x == 701, "late smart-cover was not applied before control")
assert(zero_cover:position().x == -1, "late zero placeholder moved an NPC")
assert(mob_walker:position().x == -1, "first-update sweep touched a non-animpoint scheme")
...
print("v108 deferred smart-cover placement smoke test: PASS")
