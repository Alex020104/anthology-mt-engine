...
on_game_start()
assert(state_mgr_animation.animation.add_anim == wrapped, "v117 wrapped add_anim twice")

local jup_actor, jup_calls = make_npc(1, "jup_b219_actor")
local jup_state = manager()
local idle_state = { prop = { moving = false } }
local moving_state = { prop = { moving = true } }
wrapped({ npc = jup_actor, mgr = jup_state }, "pri_a15_idle_none", idle_state)
assert(#jup_calls == 1 and jup_calls[1].local_animation == false,
	"Jupiter non-moving idle did not consume the authored absolute ground anchor")
...
for index, section in ipairs(pri_sections) do
	local pri_npc, pri_calls = make_npc(20 + index, section)
	wrapped({ npc = pri_npc, mgr = manager() }, "pri_a15_idle_none", idle_state)
	assert(#pri_calls == 1 and pri_calls[1].local_animation == false,
		"Pripyat idle anchor was not absolute for " .. section)
end

current_level = "jupiter"
local jup_azot, azot_calls = make_npc(40, "jup_b219_stalker_tech")
wrapped({ npc = jup_azot, mgr = manager() }, "pri_a15_idle_strap", idle_state)
assert(#azot_calls == 1 and azot_calls[1].local_animation == false,
	"Jupiter Azot idle did not consume the authored absolute ground anchor")
...
assert(#unrelated_calls == 1 and unrelated_calls[1].local_animation == true,
	"v117 changed an unrelated NPC anchor")
...
assert(#pas_calls == 1 and pas_calls[1].local_animation == true,
	"v117 changed an Underpass gameplay walker/remark animation")

assert(base_calls == 3, "unexpected delegation count: " .. tostring(base_calls))
assert(#log_lines >= 12, "v117 per-NPC diagnostics were not emitted")

print("PASS v117 grounded CoP root-motion smoke")
