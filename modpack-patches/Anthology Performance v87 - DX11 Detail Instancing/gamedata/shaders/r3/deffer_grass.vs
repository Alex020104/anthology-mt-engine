...
// Per-phase scale fade (moved out of the CPU fill so ONE instance buffer serves the
// main pass and every shadow pass in a frame):
//   x = 1 -> camera-distance fade: shrink by (dist_sq - y) * 0.005 past threshold y
//   x = 2 -> light fade: shrink by xz-distance-sq to light position (zw) * 0.005
//   x = 0 / unset -> no fade
...
//   inst_na  = terrain normal (xyz) + grass alpha (w)   [half4]
//   inst_sh  = sun occlusion (x) + hemi (y)             [half4, zw spare]
// The engine stores one mesh copy and issues DrawIndexedInstanced; the per-instance
// record is fetched automatically by the IA (StartInstanceLocation offsets it).
                  ...
                  float4 inst_na : TEXCOORD4, float4 inst_sh : TEXCOORD5)
{
	...
	// Per-phase fade: scale the blade around its pivot (matches the legacy CPU-side
	// scale fade that used to be baked into the per-instance matrix)
