...
uniform float4 heatvision_params1;
uniform float4 heatvision_params2;
...
float3 infrared(gbuffer_data gbd, float2 HPos, float2 Tex0) 
{
    ...
    if (hotness.r > 0.0) 
    {
        ...
        int samples = lerp(heatvision_params2.x, heatvision_params2.y, smoothstep(0.0, heatvision_params2.z, depth));
        ...
        if (projection <= 0.f) {
            ...
            float step1 = heatvision_params1.y;
            float step2 = heatvision_params1.z;
            float step3 = heatvision_params1.w;
        ...
        else
        {
            ...
            float3 min_color = lerp(color_background_min, COLOR_FAR_MIN - heatvision_params2.w*float3(0.0, 0.0, 0.03), arg);
            float3 max_color = lerp(color_background_max, COLOR_FAR_MAX - heatvision_params2.w*float3(0.0, 0.0, 0.05), arg);
    ...
    float4 jitter = float4(
					frac(sin(dot(Tex0, float2(12.0, 78.0) + (timers.x) )) * 12345.0),
					frac(sin(dot(Tex0, float2(12.0, 78.0) + (timers.x) )) * 67890.0),
					frac(sin(dot(Tex0, float2(12.0, 78.0) + (timers.x) )) * 78372.0),
					frac(sin(dot(Tex0, float2(12.0, 78.0) + (timers.x) )) * 37857.0));
    

    if (heatvision_params2.w == 1.0) 
    {
        mixed += jitter.y * (0.12);
